(function(){
  ...
  // Preloader: real asset readiness, not decorative delay.
  const preload=[ambient,...Object.values(sfx)].filter(Boolean);
  let loaded=0;
  const tick=(label)=>{ loaded++; const n=Math.round(loaded/preload.length*100); bar.style.width=n+'%'; pct.textContent=String(n).padStart(2,'0')+'%'; status.textContent=label; if(loaded===preload.length){start.disabled=false;status.textContent='AMBIENTE PRONTO';}};
  preload.forEach(a=>{ if(a.readyState>=2) tick('ASSET PRONTO'); else { a.addEventListener('canplaythrough',()=>tick('ASSET PRONTO'),{once:true}); a.addEventListener('error',()=>tick('FALLBACK ATIVO'),{once:true}); a.load(); }});
  setTimeout(()=>{if(!started && start.disabled){loaded=preload.length;bar.style.width='100%';pct.textContent='100%';status.textContent='AMBIENTE PRONTO';start.disabled=false;}},9000);
  const bootClock=setInterval(()=>{ if(loaded<preload.length){const fake=Math.min(preload.length-1,loaded+Math.random()>.65?1:0); if(fake>loaded){tick('PREPARANDO CENA');}} else clearInterval(bootClock);},500);
  ...
  Object.values(worlds).forEach(()=>{});
  worlds.forEach(w=>{w.renderer.domElement.style.cursor='crosshair'; w.renderer.domElement.addEventListener('click',e=>{const rect=w.renderer.domElement.getBoundingClientRect();mouseNdc.x=((e.clientX-rect.left)/rect.width)*2-1;mouseNdc.y=-((e.clientY-rect.top)/rect.height)*2+1;raycaster.setFromCamera(mouseNdc,w.camera);const hit=raycaster.intersectObjects([w.core],true)[0];if(hit&&hit.object.userData.hotspot){const key=w.kind==='intro'?'cid':w.kind==='defense'?'defense-depth':w.kind==='attack'?'api':w.kind==='privacy'?'lgpd':w.kind==='dev'?'sast':w.kind==='threat'?'malware':w.kind==='user'?'social-engineering':'incident-response';openTopic(key);}});});
