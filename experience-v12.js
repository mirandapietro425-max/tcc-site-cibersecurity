...
(() => {
  ...
  start?.addEventListener('click',()=>{ if(started)return; started=true; setAudio(true); if(hud)hud.hidden=false; playSfx('transition',.22); if(window.gsap) gsap.to(boot,{opacity:0,duration:1,ease:'power2.inOut',onComplete:()=>boot.remove()}); else {boot.classList.add('is-done');setTimeout(()=>boot.remove(),1100);} });
  ...
  function render(){
    ...
    worlds.forEach(w=>{
      ...
      if(w.model){const active= w.chapter?.classList.contains('is-active'); const pulse=1+Math.sin(t*1.25+w.kind.length)*.025; w.model.scale.setScalar((w.model.userData.baseScale||1)*pulse); w.model.rotation.y += .0015; w.model.position.y += (Math.sin(t*.75+w.kind.length)*.004);}
      r.render(w.scene,w.camera);
    });
    requestAnimationFrame(render);
  }
  worlds.forEach((w)=>{const id=w.renderer.domElement.id; w.chapter=document.querySelector(`[data-model][id]`)||null;});
  render();
