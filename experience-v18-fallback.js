(() => {
  'use strict';
  // V53: true fallback only. It waits for the primary V21 module before binding scroll/resize.
  ...
  const animateText = chapter => {
    if (!chapter || !active || primaryLoaded()) return;
  ...
  const update = () => {
    ticking = false;
    if (!active || primaryLoaded()) return;
    ...
    if (current !== window.__cyberShieldActiveScene) {
      window.__cyberShieldOnSceneChange?.(current);
      window.__cyberShieldAnimateScene?.(chapters[current]);
      window.__cyberShieldActiveScene = current;
    }
  };
  ...
  // Let the module loader win. Only become active if it failed to initialize.
  addEventListener('load', () => setTimeout(() => {
    if (!primaryLoaded()) activateFallback();
  }, 1200), {once:true});
