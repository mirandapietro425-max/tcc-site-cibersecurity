(() => {
  ...
  let active=0, mode='threat', audio=$('#chain-audio'), soundOn=false, userMuted=false;
  ...
  const soundButtons=$$('.chain-sound');
  function syncSound(){ soundButtons.forEach(btn=>{btn.textContent=`Som da sequência · ${soundOn?'ON':'OFF'}`;btn.setAttribute('aria-pressed',String(soundOn));}); }
  async function startSound(){
    if(!audio || userMuted) return false;
    try{ audio.loop=true; audio.volume=.09; await audio.play(); soundOn=true; syncSound(); return true; }catch(_){ return false; }
  }
  function stopSound(){ if(audio) audio.pause(); soundOn=false; syncSound(); }
  soundButtons.forEach(btn=>btn.addEventListener('click',async()=>{ if(soundOn){userMuted=true;stopSound();}else{userMuted=false;await startSound();} }));
  // Unmuted autoplay is best-effort: browsers can block it until the first gesture.
  startSound();
  const unlock=async()=>{ if(!userMuted && !soundOn){await startSound();} if(soundOn){window.removeEventListener('pointerdown',unlock,true);window.removeEventListener('keydown',unlock,true);window.removeEventListener('touchstart',unlock,true);} };
